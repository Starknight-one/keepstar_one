...
V4 read side + curator browse) without making code changes.
...
- Read EXPERTISE first for the mental model (schema, write pipeline, V4 read methods, Shopify, hot files).
...
- Two-sided architecture (admin write + V4 read, same Postgres)
...
- V4 read methods (ListProducts, VectorSearch, GetMasterProduct, GenerateCatalogDigest)
...
- V4 read tier2 fallback → read `project_v4/.../postgres_catalog.go GetMasterProduct / ListProducts / VectorSearch`
- Schema column / index → read `internal/adapters/postgres/catalog_migrations.go`

### Step 3 — Answer

- Direct answer first.
- File paths with line numbers — be EXPLICIT about which side: `project_admin/...` (write) vs `project_v4/.../postgres_catalog.go` (read).
- For "why is this listing not appearing in chat" / "why does merge skip this product" — walk: ingest → harvest → discovery artifact → match cascade → V4 read filter.
- Mention the related expert (`pipeline-agents` for chat reads, `engine-v4` for atom binding, `admin` for handler/auth, `curator` for promote/junk) when crossing layers.
...
- DO NOT introduce render concerns (Formation, atoms) — that's `engine-v4` / `widget`.
