...
func (uc *PipelineExecuteUseCase) Execute(ctx context.Context, req PipelineExecuteRequest) (*PipelineExecuteResponse, error) {
	...
	if formation != nil && formation.Mode != domain.FormationTypeSingle && uc.engine != nil {
...
// Uses engine_v4 detail preset to generate a template formation from a single dummy item.
func (uc *PipelineExecuteUseCase) buildAdjacentTemplates(state *domain.SessionState) (map[string]*domain.FormationWithData, *domain.StateData) {
	...
	// Product detail template via V4 engine (use first product as template source)
	if len(state.Current.Data.Products) > 0 {
		...
		if output.Formation != nil {
			templates[string(domain.EntityTypeProduct)] = output.Formation
		}
	}

	// Service detail template via V4 engine (use first service as template source)
	if len(state.Current.Data.Services) > 0 {
		...
		if output.Formation != nil {
			templates[string(domain.EntityTypeService)] = output.Formation
		}
	}
