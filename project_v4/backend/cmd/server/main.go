...
import (
	...
	openaiAdapter "keepstar_v4/internal/adapters/openai"
	"keepstar_v4/internal/adapters/postgres"
	"keepstar_v4/internal/config"
	engine_v4 "keepstar_v4/internal/engine_v4"
	"keepstar_v4/internal/handlers"
...
func main() {
	...
	// Initialize field definition adapter
	...
	if toolRegistry != nil && stateAdapter != nil && cacheAdapter != nil {
		pipelineUC = usecases.NewPipelineExecuteUseCase(llmClient, stateAdapter, cacheAdapter, traceAdapter, catalogAdapter, toolRegistry, eng, agent2UC, appLog)
	...
	if stateAdapter != nil {
		expandUC := usecases.NewExpandUseCase(stateAdapter, eng)
		backUC := usecases.NewBackUseCase(stateAdapter, eng)
	...
	if stateAdapter != nil {
		debugHandler = handlers.NewDebugHandler(stateAdapter, cacheAdapter, metricsStore, eng)
	}
	...
	if catalogAdapter != nil {
		testbenchHandler := handlers.NewTestbenchHandler(catalogAdapter, eng)
