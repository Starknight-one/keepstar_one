#!/bin/bash
# Start chat (backend + frontend + inspector)
...
sleep 4
curl -sf http://localhost:8080/health > /dev/null && echo "Chat started: backend :8080, frontend :5173, inspector :3457" || echo "WARNING: backend health check failed"
