...
- [ ] **Фаза 2 — токены текут из канваса.** `translate.go` перестаёт ронять `variables/themes`; сохранение витрины пишет токены в тему тенанта; биндинг canvas-переменная → `--kw-*` ключ.
  - *Проверка:* поменял цвет в канвасе → сохранил → виджет перекрасился.
- [ ] **Фаза 3 — витрина дизайн-системы.** Style guide board (все токены + компоненты), вход из админки «Design System» → открывается в канвасе. Дорабатываем VariableEditor по необходимости.
  - *Проверка:* видно всю систему на доске, клик по элементу → правка → разъехалось.
- [ ] **Фаза 4 — правка пресетов в канвасе + 14 сидов.** Кнопка «открыть в канвасе» (канвас-пресеты — через `canvas_registrations`); отрисовка 14 встроенных и регистрация.
  - *Проверка:* открыл product_card в канвасе, подкрутил, перерегал, виджет показал новое.
- [ ] **Фаза 5 — публичная полка** (каркас, покупка отложена). Таблицы полки + провенанс; «опубликовать» / «скачать = копия в тенант».
  - *Проверка:* опубликовал тему, другой тенант скачал, правит независимо.
