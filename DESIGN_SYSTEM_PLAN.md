...
# Архитектурный вывод по Фазе 4 (декомпилятор) — обсуждено с owner 2026-06-25

**Симптом (owner на тесте):** сложные пресеты (detail / accordion / comparison), открытые в канвасе через «Open in canvas» / «View», выглядят почти одинаково — «картинка + название + цена + текст», как обычная карточка. Простой `product_card` конвертится нормально.

**Корень — НЕ баг и НЕ «JSON↔JSON». У нас два РАЗНЫХ языка:**
- **v9** (Pencil-like) — формат ДИЗАЙНА: фреймы/фигуры/декор/геометрия/цвета. Про «как выглядит».
- **v5** — РАНТАЙМ виджета: `fieldBinding` (привязка к данным каталога), `replicate`, аккордеон, компоненты-`ref`, action-слоты. Про «как работает с данными».

Они пересекаются лишь частично. `translate.go` — намеренно лосси **компилятор** v9→v5 (выкидывает декор/геометрию, округляет размеры в «ведёрки», угадывает биндинги по имени слоя). «Декомпиляция» = обращение лосси-компилятора → уничтоженную информацию вернуть нечем (как RAW из JPEG / исходник из бинарника). Pencil круглит лосслесс, потому что НИКОГДА не выходит из своего формата (v9→v9).

**Почему сложные пресеты «шляпа», две причины:**
1. v5-only концепты (`collapsible/defaultOpen`, компоненты-`ref`, привязка к данным) **в v9 не существуют как слово** → схлопываются (аккордеон → плоский стек; brand-badge-компонент → выкинут; нераспознанный текст падает в биндинг `name` → в аккордеоне «Product Name» трижды).
2. 14 сидов написаны руками **сразу в v5** — v9-исходника у них никогда не было; декомпайл его выдумывает приближённо.

**Решение (это и был исходный path A, от которого decompiler-шорткат увёл):**
**v9 — источник правды, v5 — скомпилированный артефакт.** Дизайнишь в канвасе (v9) → компилируешь в v5 для виджета → правишь всегда v9-исходник (лосслесс). Декомпайл не нужен.
- **Декомпайл оставить как разовый bootstrap:** генерим стартовый v9 из каждого v5-сида, руками доводим 3-4 сложных, **сохраняем как v9-исходник** → дальше живём в v9.
- Параллельно можно поднять fidelity компилятора `translate.go` (декор фреймов фон/радиус/тень виджет уже умеет — сделано в product-card redesign). Но рантайм-разрыв (аккордеон/компоненты/данные) это НЕ закроет — он принципиальный.

**Статус: ОТКРЫТО (ждёт owner).** Выбор пути не зафиксирован:
1. bootstrap → курируем v9-исходники → v9 = source of truth (чисто, но работа);
2. принять упрощёнку, «Open in canvas» = редактор цвет/текст/лейаут + честный баннер «simplified copy»;
3. сложные пресеты сделать view-only/advanced, не давать редактировать через канвас.
