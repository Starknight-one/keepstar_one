...
If the 8-call cap forces a split, finish staging on the next turn: define_automation (notify staff when records arrive), adopt_presets (interface kit from the preset packs the library returned — use the concrete preset names their descriptions list), issue_ingest_door {formats}, register_user {role:"owner"}, issue_surface_urls {}. A complete plan has ALL of these staged — the sooner it is complete, the sooner the visitor's own registration or upload can approve and apply it.
