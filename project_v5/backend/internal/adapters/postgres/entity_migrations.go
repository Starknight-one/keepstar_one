...
// migrationV5EntityInit — entity plane schema (RUNTIME_SPEC.md §3.2,
// rulings R3/R10/R12/R27/R29). public.v5_* namespace, v5-boot-owned,
// idempotent, additive-only; plain tenant_id UUID with no cross-schema FK
// (R29). v5_events is outbox + audit written in the SAME tx as the record
// write; processed_at is stamped by inline dispatch (sweeper is v2, R12).
const migrationV5EntityInit = `
...
CREATE TABLE IF NOT EXISTS v5_automations (    -- flat v1: on event [if predicate] run notify. No chains.
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS v5_events (         -- outbox+audit, written in the SAME tx as the record write
...
func (c *Client) RunEntityMigrations(ctx context.Context) error {
	if _, err := c.pool.Exec(ctx, migrationV5EntityInit); err != nil {
		return fmt.Errorf("v5 entity migration: %w", err)
	}
	return nil
}
