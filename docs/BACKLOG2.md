...
**Не покрыто:**
- [~] Сравнение ("сравни два крема") — бэкенд preset есть, фронт ComparisonTemplate не рендерится (mode не доходит)
...
Сейчас агент уже может влиять на: какие поля показать, display стиль каждого поля (h1/h2/price-lg/badge...), размер виджета. Но не всё доезжает до экрана — фронт местами хардкодит своё (порядок слотов в JSX, размеры карточек в CSS).
...
### 3. Сделать красивее — КРИТИЧНО, СЛОМАНО

**Текущее состояние после попытки полиша 2026-02-21:**

CSS виджетов в хаосе. Попытка убрать хардкод ширин привела к раздуванию карточек. Нужна полная переработка:

- [ ] **Грид раздут:** карточки без max-width заполняют всю ячейку → огромные шрифты и картинки. Нужен баланс fixed/fluid
- [ ] **Comparison не рендерится:** mode=comparison не доходит до ComparisonTemplate, рендерится как list с гигантскими фотками
- [ ] **Sticky счётчик уродливый:** "13 товаров" плашка сверху — парашно выглядит, нужен редизайн или убрать
- [ ] **Overlay ширина:** `widget-display-area > * { width: 100% }` — слишком агрессивно, нужен max-width
...
*Последнее обновление: 2026-02-21 (frontend polish попытка — бэк ок, фронт сломан, нужна переработка CSS)*
