...
### ~~Логирование — полное покрытие~~ DONE
Реализовано в 7 фазах: Postgres `request_logs` + LogAdapter, Logger extension (With/FromContext), HTTP Middleware (request_id + SpanCollector + async persistence), Span инструментация всех слоёв (handlers → usecases → adapters), Retention 3 дня, Admin backend logging, Frontend logger. ~32 файла. См. `done-logging-full-coverage.md`.
...
### ~~Навигационная панель~~ PARTIALLY DONE
Степпер (vertical nav trail) реализован и встроен в чат-колонку. Trail-модель: каждый шаг — append, back/goTo = push копии. Визуально: ghostly minimal, полупрозрачные точки/линии. TODO: подчистить логику степпера (слишком длинные labels, дублирование шагов при back), лайки/сохранение товаров, share, compare.
