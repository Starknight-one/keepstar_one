...
| Preset storage location | `v5_presets` / `v5_preset_versions` **and** `v5_components` / `v5_component_versions` are the **final shape**, not a stopgap. The separate-v9-microservice idea did NOT materialise: the write-side client shipped inside the rebuilt **keepstar-admin** canvas (`internal/canvas/translate.go` translates canvas docs; `handlers/handler_canvas_presets.go` writes `v5_presets` / `v5_preset_versions` as a DB client; preview + prompt-cache-invalidate via V5's `/api/v1/internal/presets/*`). | Stream A defined the schema; keepstar-admin's canvas consumes it. | **Shipped** — write-side client = keepstar-admin canvas. Canvas roadmap is now governed by root `CANVAS_MASTER_PLAN.md`. |
...
| ~~Vector search / EmbeddingPort~~ | ~~V5 `catalog_search` is keyword-only (V5 ListProducts multi-word AND-logic ILIKE). V4's hybrid pgvector + RRF path is not ported. Tool schema keeps `vector_query` so V4 prompt copies byte-identically; executor falls back to keyword.~~ | ~~Chunk 7 stayed lean — embeddings + vector index + RRF would have doubled the chunk size.~~ | **Closed** — hybrid search shipped: `tools/tool_catalog_search.go` embeds `vector_query` via `EmbeddingPort` (`adapters/openai/embedding_client.go`, text-embedding-3-small @ 384 dims) in parallel with keyword SQL, pgvector cosine (`postgres_catalog_vector.go`), RRF merge k=60 (keyword 1.5×, 2.0× with structured filters). Remaining truth: `OPENAI_API_KEY` is optional — blank or embedding error → keyword-only fallback. |
...
|---|---|---|
| ~~V5 sessions write to Neon `v5_*` tables in the same DB as V4. If a frontend confuses session ids across the two engines and reads `current_template` expecting Formation, it will see scene-graph and break.~~ | ~~Cross-engine read in same env~~ | **Moot** — V5 is exposed over HTTP since chunk 14 behind its own `/api/v1` prefix + `WithTenant` gate, and V4 itself was deleted (`15ca2fe`); there is no second engine to confuse sessions with. |
...
| ~~**V5 prompt+tool prefix below stable cache threshold**~~ — chunk-5.5 measurement: V5 system+tools = 3001 tokens, V4 = 6449 tokens. Anthropic Haiku documented min is 2048, but Vlad's V4-prod experience says ≥ 4500 is needed for stable caching. | ~~Chunk-6b prompt-builder ships an under-sized prompt~~ | **RESOLVED** — chunk 6b shipped and the bar is cleared: prod smoke (chunk 14) measured `cache_read=7546` tokens on the system+tools prefix, and the prompt has grown since (new system presets + rectangle/line/icon_font vocabulary). Measurement harness lives at `internal/measure/tokens/measurement_test.go` (build tag `tokens`; the old `internal/engine/tokens/` path no longer exists) — re-run as the prompt evolves. |
