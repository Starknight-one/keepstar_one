...
| Agent2 prompt-builder with `<fields>` block | Originally scoped for chunk 4 alongside the first preset; pushed to chunk 5. | Chunk 4 already absorbs preset storage + replicate-op + image binding; adding the prompt builder would blow scope. The builder's natural pair is the first real Agent2 turn, which lands in chunk 5. | Chunk 5 (first end-to-end Agent2 call against V5) |
