...
// validateInput sanitizes tool input values, stripping invalid entries (V1 only)
...
// executeV1 runs the legacy v1 engine pipeline.
func (t *VisualAssemblyTool) executeV1(ctx context.Context, toolCtx ToolContext, input map[string]interface{}) (*domain.ToolResult, error) {
	degraded := false
